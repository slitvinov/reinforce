...
class ReinforceAgent:
    ...
    def eligibility_vector(self, a, s):
        return self.x(s, a) - self.pi(self.x(s)) * (
            self.x(s, ShortCorridor.left) + self.x(s, ShortCorridor.right))
...
def experiment(returns, alpha_index, alpha, run_num, random_seed, num_episodes,
               max_timesteps):
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description=
        'A script to generate figure 13.1 from Sutton and Barto (2nd Ed.)')
    parser.add_argument('--alphas',
                        type=float,
                        nargs='*',
                        default=[2**-12, 2**-13, 2**-14],
                        help='Policy step sizes')
    ...
    parser.add_argument('--num_episodes',
                        type=int,
                        default=1000,
                        help='The number of episodes per run')
    parser.add_argument(
        '--max_timesteps',
        type=int,
        default=1000,
        help='The maximum number of timesteps allowed per episode')
    ...
    random_seeds = [
        np.random.randint(low=0, high=2**32) for run in range(args.num_runs)
    ]
    ...
    if os.path.exists('returns_13_1.npy'):
        ...
        returns = np.memmap('returns_13_1.npy',
                            shape=(len(args.alphas), args.num_runs,
                                   args.num_episodes),
                            dtype=np.int16,
                            mode='r')
    else:
        ...
        Parallel(n_jobs=args.num_cpus, verbose=10)(
            delayed(
                experiment)(returns, alpha_index, alpha, run_num, random_seed,
                            args.num_episodes, args.max_timesteps)
    ...
    for alpha_index, alpha in enumerate(args.alphas):
        ...
        p = plt.plot(
            np.arange(args.num_episodes),
            means,
            label='2^{}'.format(int(np.log2(
                alpha))))  # keep reference for colour-matching with errorbars.

        if args.confidence_intervals:
            # Plot 95% confidence intervals:
            sems = st.sem(returns[alpha_index], axis=0)
            confs = sems * st.t.ppf((1.0 + 0.95) / 2, args.num_runs - 1)
            ax.errorbar(np.arange(args.num_episodes),
                        means,
                        yerr=[confs, confs],
                        color=p[0].get_color(),
                        alpha=.15)
    ...
    ax.set_ylim(-90, -10)
    ax.set_title('Performance of REINFORCE (averaged over {} runs)'.format(
        args.num_runs))
